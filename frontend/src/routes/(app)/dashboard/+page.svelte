<script lang="ts">
	...
	let maxChartValue = $derived(Math.max(1, ...chartData.map((item) => item.value)));
	...
	async function loadDashboard(isRefresh = false) {
		...
		error = '';
		try {
			const response = await fetch(`http://localhost:8081/api/v1/dashboard?year=${dashboardYear}`, {
				method: 'GET',
				headers: {
			...
			dashboardYear = result.data.year;
			statistics = result.data.statistics.map((item) => ({
				...item,
				icon: iconMap[item.key] ?? FileText
			}));
			chartData = result.data.chartData;
			recentActivity = result.data.recentActivity.map((item, index) => ({
				...item,
				color: activityColors[index % activityColors.length]
			}));
			notifications = result.data.notifications;
			if (chartData.length > 0 && !chartData.some((item) => item.month === selectedMonth)) {
			...
			error = err instanceof Error ? err.message : 'Tidak dapat mengambil data dashboard';
		} finally {
			loading = false;
			refreshing = false;
		}
	...
	function calculateArchiveRatio() {
		...
		const archived = getStatisticValue('archive');
		if (total <= 0) return 0;
		return Math.min(100, Math.round((archived / total) * 100));
	}
</script>
...
<div class="mx-auto w-full max-w-[1500px] px-5 py-7 md:px-8 lg:px-10">
	<div class="flex flex-col justify-between gap-5 md:flex-row md:items-start">
		<div in:fly={{ y: 18, duration: 450 }}>
			<h1 class="text-2xl font-bold tracking-tight md:text-[28px]">Dashboard</h1>
			<p class="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">
				Pantau ringkasan aktivitas persuratan, proses disposisi, perkembangan dokumen, arsip, serta
				aktivitas pengguna Sistem Informasi Persuratan dalam satu halaman.
			</p>
		</div>
	...
	{:else if error}
		<Card.Root class="mt-7 border-destructive/30 shadow-none">
			<Card.Content class="flex flex-col items-center justify-center py-12 text-center">
				...
				<Button class="mt-5" onclick={() => loadDashboard(true)}>
	...
	{:else if activeTab === 'Overview'}
				...
				<div in:fly={{ y: 22, duration: 450, delay: 200 + index * 70 }}>
					<button
						...
						class={[
							...
							selectedStat === item.key ? 'border-foreground/30 shadow-sm' : 'border-border'
						]}
					>
		...
		<section class="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1.7fr)_minmax(340px,.8fr)]">
			<div class="min-w-0" in:fly={{ y: 25, duration: 500, delay: 500 }}>
				<Card.Root
					class="h-full overflow-hidden shadow-none transition-shadow duration-200 hover:shadow-sm"
				>
					<Card.Header>
					...
					<Card.Content>
						<div class="relative h-[330px]">
							<div
								class="absolute top-0 bottom-8 left-0 flex w-12 flex-col justify-between text-right text-[10px] text-muted-foreground"
							>
							...
							<div class="absolute top-0 right-0 bottom-0 left-16 grid grid-cols-12 gap-2">
						...
						{:else}
							{#each recentActivity as activity, index}
								...
								>
									...
									<p
										class="max-w-24 shrink-0 truncate text-[11px] font-semibold transition-transform duration-200 group-hover:translate-x-0.5"
									>
										{activity.value ?? '-'}
									</p>
								</div>
				...
				<Card.Root class="h-full shadow-none">
					...
					<Card.Content>
						<div class="flex min-h-52 flex-col items-center justify-center">
							<div
								class="group flex size-36 items-center justify-center rounded-full border-[14px] border-muted transition-all duration-300 hover:scale-105 hover:border-foreground/20"
							>
		...
		<section class="mt-7 grid gap-4 md:grid-cols-3">
			{#each reports as report, index}
				<div in:fly={{ y: 22, duration: 400, delay: index * 80 }}>
					<Card.Root
						class="group h-full shadow-none transition-all duration-200 hover:-translate-y-1 hover:shadow-md"
					>
						<Card.Header>
							<div
								class="mb-3 flex size-10 items-center justify-center rounded-lg bg-muted transition-all duration-200 group-hover:bg-foreground group-hover:text-background"
							>
				...
				<Card.Content class="divide-y p-0">
					...
					{:else}
						{#each notifications as item, index}
							...
							>
								<div
									class="mt-0.5 flex size-9 shrink-0 items-center justify-center rounded-full bg-muted transition-all duration-200 group-hover:bg-foreground group-hover:text-background"
								>
									<Bell class="size-4" />
								</div>
								<div class="min-w-0 flex-1">
								...
								<ArrowUpRight
									class="size-4 shrink-0 text-muted-foreground transition-transform duration-200 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 group-hover:text-foreground"
								/>
							</button>
						{/each}
					{/if}
